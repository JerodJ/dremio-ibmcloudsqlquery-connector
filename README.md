# CData Dremio ARP Connector for IBM Cloud SQL Query

The IBM Cloud SQL Query connector allows Dremio to connect to and query data in IBM Cloud SQL Query. This can then allow you to build custom reports, dashboards, or even just ad-hoc SQL via your client tool of choice. Note that it does require a third-party JDBC driver that is not free, but does allow a free trial.
...
3. Download and install the [IBM Cloud SQL Query JDBC Driver from CData](https://www.cdata.com/drivers/ibmcloudsql/download/jdbc)* and copy the JAR file to the /jars/3rdparty/ directory of your Dremio installation. For example:
